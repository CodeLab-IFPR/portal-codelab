...
<div class="admin-ui-page admin-ui-page-narrow">
    <div class="admin-ui-intro">
        <div class="admin-ui-intro-copy">
            <h1 class="admin-ui-title">Editar mídia</h1>
        </div>
            ...
            <div class="admin-ui-grid">
                <div class="admin-ui-field">
                    ...
                    <select class="admin-ui-input @error('tipo') is-invalid @enderror" id="tipo" name="tipo" required>
                        ...
                        <option value="imagem" {{ old('tipo', $galeria->tipo) === 'imagem' ? 'selected' : '' }}>Imagem</option>
                        <option value="video" {{ old('tipo', $galeria->tipo) === 'video' ? 'selected' : '' }}>Vídeo do YouTube</option>
                    </select>
                    @error('tipo')
                ...
                <div id="file-input" class="admin-ui-field d-none">
                    ...
                    @if($galeria->tipo === 'imagem')
                        ...
                        <div class="admin-ui-thumbnail admin-ui-thumbnail-preview">
                            <img src="{{ asset($galeria->caminho) }}" alt="{{ $galeria->titulo }}">
                        </div>
                    @endif
                    @error('file')
                        <div class="admin-ui-error">{{ $message }}</div>
                ...
                <div class="admin-ui-field">
                    ...
                    <input type="text" class="admin-ui-input @error('titulo') is-invalid @enderror"
                        id="titulo" name="titulo" value="{{ old('titulo', $galeria->titulo) }}" placeholder="Título da mídia" required>
                    @error('titulo')
                        <div class="admin-ui-error">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="admin-ui-field">
                    ...
                    <textarea class="admin-ui-textarea @error('descricao') is-invalid @enderror"
                        id="descricao" name="descricao" rows="3" placeholder="Descrição da mídia">{{ old('descricao', $galeria->descricao) }}</textarea>
                    @error('descricao')
                        <div class="admin-ui-error">{{ $message }}</div>
                    @enderror
                </div>
